...
export default async function Blog() {
  ...
  return (
    <div className="anim-opacity">
      <div className="h-20 bg-gradient-to-tr from-sky-100 to-indigo-300"></div>
      <div className="mx-auto mt-8 max-w-[1400px] px-4 lg:mt-12">
        <h1 className="mb-3 border-b border-orange-300 pb-2 text-lg font-semibold uppercase">
          Popolarne posty
        </h1>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-2 lg:grid-cols-4 ">
          {data.map((d, i) => (
            <Link
              href={`/blog/${d.slug}`}
              key={i}
              className="group overflow-hidden rounded-xl lg:mb-5"
            >
              <div className="relative h-28 w-full lg:h-40">
                <Image
                  className="object-cover brightness-90 transition-all duration-300 group-hover:scale-[.97] group-hover:rounded-xl group-hover:brightness-105"
                  src={d.img.url}
                  alt={d.title}
                  priority={i < 4}
                  fill
